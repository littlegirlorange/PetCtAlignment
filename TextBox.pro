...
CASE event.ID OF
   ...
   ELSE: BEGIN
	  ...
	  for i=0, n_elements( info.wTextArray )-1 do begin
      	  Widget_Control, info.wTextArray[i], Get_Value=theText
      	  (*info.ptr).textArray[i] = theText[0]
      endfor
      (*info.ptr).cancel = 0
...
IF N_Elements(labelArray) EQ 0 THEN labelArray[0] = ""
IF N_Elements(valueArray) EQ 0 THEN valueArray[0] = ""
...
wBaseArray = intarr( n_elements( valueArray ) )
wLabelArray = wBaseArray
wTextArray = wBaseArray

for i=0, n_elements( labelArray )-1 do begin
	if labelArray[i] ne "" then begin
		wBaseArray[i]	= Widget_Base(tlb, Row=1)
		wLabelArray[i]	= Widget_Label(wBaseArray[i], Value=labelArray[i])
		wTextArray[i]	= Widget_Text(wBaseArray[i], /Editable, Scr_XSize=xsize, Value=valueArray[i])
	endif
endfor
...
ptr = Ptr_New({textArray:strarr( n_elements( labelArray ) ), cancel:1})

   ; Store the program information:

info = {ptr:ptr, wTextArray:wTextArray, cancelID:cancelID}
