...
;
;		071205 MK:
;		Moved the scale operation from the end to before translation back to 0,0.
;		Invert the t3dmat before applying to the coords so that the volume is
;		transformed, not the coordinates.
;
;		090724 MK:
;		Changed the data type of the returned array from int to match the
;		input array type.
;
;-
;###########################################################################
...
FUNCTION Transform_Volume, volume, PIXSIZE=pixSize, Rotation=rotation, $
...
vol_t = make_array( sx, sy, sz, TYPE=s[4] )
;vol_t = volume*0
...
IF N_Elements(t3dmat) EQ 0 THEN begin
   ...
   IF N_Elements(scale) EQ 0 THEN scale =[1,1,1]
	if n_elements( pixSize ) ne 3 then pixSize = [1,1,1]

   T3D, /Reset, Translate = -centre_rotation
   T3D, SCALE=pixSize
   T3D, Rotate = rotation
   T3D, Scale = scale/pixSize
   T3D, Translate = (centre_rotation + translate)
   t3dmat = !P.T
...
ENDIF
 ...
 FOR j=0L,(st-1),buffer_size DO BEGIN
   ...
   coords = Temporary(coords) # invert(t3dmat)
